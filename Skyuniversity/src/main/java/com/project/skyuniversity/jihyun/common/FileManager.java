package com.project.skyuniversity.jihyun.common;
...
public class FileManager {
	...
	public String doFileUpload(byte[] bytes, String originalFilename, String path) throws Exception {
		...
		newFilename = String.format("%1$tY%1$tm%1$td%1$tH%1$tM%1$tS", Calendar.getInstance()); //년월일시분초
	    newFilename += System.nanoTime(); 													   // 년월일시분초 + 나노초
	    newFilename += fileExt; 															   // 년월일시분초 + 나노초 + 확장자명
	    ...
	    // 자바에서는 File 클래스를 사용하여 폴더 또는 파일을 생성 및 관리하게된다.
	    ...
	    // pathname은 예를 들면 C:\NCS\workspace(spring)\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\Board\files\20201208092715353243254235235234.png 이다
	    ...
	    fos.write(bytes); // write(byte[] bytes) 메소드가 해당 경로 파일명(pathname)에 실제로 데이터 내용(byte[] bytes)을 기록해주는 일을 하는 것이다.
	    fos.close(); 	// 생성된 FileOutputStream 객체 fos 가 더이상 사용되지 않도록 소멸 시킨다.
	...
	public boolean doFileDownload(String saveFilename, String originalFilename, String path, HttpServletResponse response) {
		 ...
		 try {
			 ...
			 // download할 파일명(pathname)을 가지고 File 객체를 생성한다.
			 
			 if(file.exists()) { //실제로 다운로드 할 파일이 존재한다면
				...
				// 다운로드할 파일의 종류에 따라 Content-Type 을 지정해주어야 한다.
				 ...
				 // 클라이언트가 다운로드 해야할 파일(file)을 읽어들이기위해 new FileInputStream(file)객체를 생성함
				 // 또한, 빠르게 읽어드리기 위해서 필터스트림(보조스트림) BufferedInputStream bfin 을 장착함
				 
				 ServletOutputStream souts = response.getOutputStream();
				 //ServletOutputStreamd은 다운로드 될 파일을 클라이언트로 보내어주는 출력 스트림요요 클래스이다.
				 int length = 0;
				 
				 while( (length = bfin.read(readByte, 0, 4096)) != -1) { // 0(처음)부터 4kb만큼씩 읽어와서 readByte에 넣어줌
					 /*
					   bfin.read(readByte, 0, 4096)은 다운로드 해주어야할 file에서 0(처음) 에서 4096byte 만큼 읽어들인후
					   readByte라는 변수에 읽어들인 내용을 저장시킨다.
					   그런데 다운로드 해주어야할 file에서 더이상 읽어들인 내용이 없다면 (length = bfin.read(readByte, 0, 4096))은
					   -1을 return시켜준다. 그러므로 while( (length = bfin.read(readByte, 0, 4096)) != -1)은
					   다운로드할 file을 매번 4096byte씩 읽어들인다는 말이다.
					  */
					 
					 souts.write(readByte, 0, length);
					 /*
					     readByte에 저장된 내용을 처음부터 읽어온 크기인 length만큼 ServletOutputStream souts에 기록(저장)해둔다.
					  */
				 }// end of while
					 
					 souts.flush(); // ServletOutputStream souts에 기록(저장)해둔 내용을 클라이언트 웹브라우절로 내보낸다.
					 
					 souts.close();
					 bfin.close(); //BufferedInputStream bfin 객체를 소멸시킴
				 return true; //다운로드할 파일이 존재하고 Exception이 발생하지 않으면 true 리턴
			 }// end of if()----------------------------
